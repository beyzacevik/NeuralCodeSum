...
def load_data(args, filenames, max_examples=-1, dataset_name='java',
    ...
    print('*** * * ** * ', filenames['src'])
    print('*** * * ** * ', filenames['tgt'])
    print('*** * * ** * ', filenames['src_tag'])

    df_code = pd.read_csv(filenames['src'])
    sources = [str(line) for line in df_code.code.values.tolist()]
 

    if filenames['tgt'] is not None:
        df = pd.read_csv(filenames['tgt'])
        lines = [str(line) for line in df.javadoc.values.tolist()]
        targets = lines
    else:
    ...
    print(len(sources), len(source_tags), len(targets))
